<template>
  <div id="user_register_view">
    <a-form
      :model="state.formDataUser"
      :style="{
        width: '600px',
        right: '0',
        left: '0',
    ...
    >
      <a-form-item field="userAccount" tooltip="请输入用户名" label="用户名">
        <a-input
          v-model="state.formDataUser.userAccount"
          placeholder="请输入用户名..."
          feedback
          :rules="[
            { required: true, message: '用户名是必需的' },
            { minLength: 4, message: '用户名长度至少为4' },
          ]"
      ...
      <a-form-item field="userPassword" label="密码">
        <a-input-password
          v-model="state.formDataUser.userPassword"
          placeholder="请输入密码..."
          feedback
          :rules="[
      ...
      <a-form-item field="checkPassword" label="确认密码">
        <a-input-password
          v-model="state.formDataUser.checkPassword"
          placeholder="请再次输入密码..."
          feedback
          :rules="[
            { required: true, message: '密码是必需的' },
      ...
      >
        <a-input
          v-model="state.formDataUser.userEmail"
          placeholder="请输入邮箱..."
          feedback
      ...
      <a-form-item field="emailVerifyCode" label="验证码">
        <a-input-number
          v-model="state.formDataUser.emailVerifyCode"
          placeholder="请输入验证码"
          feedback
      ...
      <a-form-item>
        ...
        <a-button style="float: right" type="text" @click="goToLogin"
          >前往登录
        </a-button>
      </a-form-item>
    </a-form>
...
async function sendVerifyCode() {
  if (!isValidEmail(state.formDataUser.userEmail)) {
    Notification.error({
      title: "警告",
      content: "邮箱格式不正确!",
      closable: true,
...
async function handleRegister() {
  ...
  let res;
  res = await UserControllerService.userRegisterUsingPost({
    ...state.formDataUser,
  });
  if (res.code !== null && res.code === 1) {
